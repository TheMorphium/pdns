...
class DNSDistTest(AssertEqualDNSMessageMixin, unittest.TestCase):
    ...
    def startDNSDist(cls):
      ...
      confFile = os.path.join('configs', f'dnsdist_{cls.__name__}.conf')
      ...
      if not cls._verboseMode and output != expectedOutput:
        raise AssertionError(f'dnsdist --check-config failed: {output}')

      logFile = os.path.join('configs', f'dnsdist_{cls.__name__}.log')
    ...
    def UDPResponder(cls, port, fromQueue, toQueue, trailingDataResponse=False, callback=None):
      ...
      while True:
        ...
        if callback:
          ...
          if response := cls._getResponse(request,
                                          fromQueue,
                                          toQueue,
      ...
      while True:
        ...
        for event in events:
          ...
          if isinstance(event, h2.events.DataReceived):
            ...
            if event.stream_id not in dnsData:
            ...
            if event.stream_ended:
              ...
              elif response := cls._getResponse(request,
                                                fromQueue,
                                                toQueue,
                                                synthesize=forceRcode):
                wire = response.to_wire(max_size=65535)
          ...
          if data_to_send := h2conn.data_to_send():
    ...
    def sendTCPQueryOverConnection(cls, sock, query, rawQuery=False, response=None, timeout=2.0):
      wire = query.to_wire() if not rawQuery else query
      if response:
          cls._toResponderQueue.put(response, True, timeout)
    ...
    def sendTCPQuery(cls, query, response, useQueue=True, timeout=2.0, rawQuery=False):
      ...
      try:
        ...
        print(f"Timeout while sending or receiving TCP data: {str(e)}")
      except socket.error as e:
    ...
    def sendTCPQueryWithMultipleResponses(cls, query, responses, useQueue=True, timeout=2.0, rawQuery=False):
      ...
      try:
        ...
        print(f"Timeout while receiving multiple TCP responses: {str(e)}")
      except socket.error as e:
        print(f"Network error: {str(e)}")
      finally:
        sock.close()
    ...
    def sendConsoleCommand(cls, command, timeout=5.0):
      ...
      readingNonce = ourNonce[:halfNonceSize] + theirNonce[halfNonceSize:]
      writingNonce = theirNonce[:halfNonceSize] + ourNonce[halfNonceSize:]
    ...
    def getDOHGetURL(cls, baseurl, query, rawQuery=False):
      wire = query if rawQuery else query.to_wire()
      param = base64.urlsafe_b64encode(wire).decode('UTF8').rstrip('=')
      return f"{baseurl}?dns={param}"
    ...
    def sendDOHQuery(cls, port, servername, baseurl, query, response=None, timeout=2.0, caFile=None, useQueue=True, rawQuery=False, rawResponse=False, customHeaders=[], useHTTPS=True, fromQueue=None, toQueue=None):
      ...
      if useQueue:
        ...
        elif not cls._fromResponderQueue.empty():
          receivedQuery = cls._fromResponderQueue.get(True, timeout)
